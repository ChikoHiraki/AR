...
# 画像ファイルを読み込む
background_image_path = directory_path+"/img/kougakubu_hare2.jpg"  
...
# houseのマテリアルを取得
#house.active_material_index = 0
#house_material = bpy.data.materials.get("HouseMaterial")
...
house_bake_texture1 = house_nodes.new(type='ShaderNodeTexImage') # ベイク用のテクスチャ
new_image1 = bpy.data.images.new("house_bake_from_dome", 1024, 1024)
...
dome.data.materials.append(DomeMaterial)
# マテリアルの設定
...
# 画像テクスチャを読み込む
dome_texture_node.image = background_image
# マテリアルのマッピングを設定
...
dome_mapping_node.inputs['Scale'].default_value[0] = -1
...
# ベイク結果を保存
output_path1 = directory_path+"/img/house_bake_from_dome.png"
#bpy.data.images['house_bake_from_dome'].save_render(filepath=output_path1)
...
# 新しいマテリアルを作成
planeMaterial = bpy.data.materials.new(name="PlaneMaterial")
...
# 画像テクスチャを読み込む
plane_image_path = directory_path+"/img/camera.png"  
...
# 建造物から地面への光をベイク
plane_diffuse_node = plane_nodes.new("ShaderNodeBsdfDiffuse")
plane_links.new(plane_texture_node.outputs['Color'], plane_diffuse_node.inputs['Color'])
plane_links.new(plane_diffuse_node.outputs['BSDF'], plane_output_node.inputs['Surface'])
...
house_bake_texture3 = house_nodes.new(type='ShaderNodeTexImage') # ベイク用のテクスチャ
new_image3 = bpy.data.images.new("house_bake_from_ground", 1024, 1024)
house_bake_texture3.image = new_image3
house_nodes.active = house_bake_texture3
...
#bpy.ops.object.bake(type='DIFFUSE')
# ベイク結果を保存
output_path3 = directory_path+"/img/house_bake_from_ground.png"
#bpy.data.images['house_bake_from_ground'].save_render(filepath=output_path3)
